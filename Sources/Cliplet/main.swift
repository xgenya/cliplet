import AppKit

MainActor.assumeIsolated {
    if ProcessInfo.processInfo.arguments.contains("--smoke-test") {
        do {
            try AppSmokeTest.run()
            exit(0)
        } catch {
            FileHandle.standardError.write(Data("Cliplet package smoke test failed: \(error)\n".utf8))
            exit(1)
        }
    }
    // Register before competing for the lock, so an early second launch cannot
    // send its reopen request before the winning process has a listener.
    let reopenRequests = AppReopenRequests()
    // A second launch asks the running copy to open its history, then exits
    // before it can register another hotkey or write to the same history.
    let instanceLock: AppInstanceLock
    do {
        let build = try AppInstanceLock.buildIdentifier()
        var acquired = try AppInstanceLock.acquire(build: build)
        var replacedRunningCopy = false
        if acquired == nil, let owner = AppInstanceLock.owner(), !owner.build.isEmpty, owner.build != build {
            DistributedNotificationCenter.default().postNotificationName(
                AppInstanceLock.replacementNotification, object: owner.token, userInfo: nil, deliverImmediately: true)
            let deadline = Date().addingTimeInterval(15)
            repeat {
                RunLoop.current.run(until: Date().addingTimeInterval(0.05))
                acquired = try AppInstanceLock.acquire(build: build)
                // Another launch may win the lock. Never ask that new owner to quit.
                if acquired != nil || AppInstanceLock.owner()?.token != owner.token { break }
            } while Date() < deadline
            guard acquired != nil else {
                FileHandle.standardError.write(Data("Cliplet is still running; update was not started.\n".utf8))
                DistributedNotificationCenter.default().postNotificationName(
                    AppInstanceLock.reopenNotification, object: nil, userInfo: nil, deliverImmediately: true)
                exit(1)
            }
            replacedRunningCopy = true
        }
        guard let acquired else {
            DistributedNotificationCenter.default().postNotificationName(
                AppInstanceLock.reopenNotification, object: nil, userInfo: nil, deliverImmediately: true)
            exit(0)
        }
        instanceLock = acquired
        if replacedRunningCopy { reopenRequests.receive() }
    } catch {
        FileHandle.standardError.write(Data("Cliplet could not establish its instance lock: \(error)\n".utf8))
        exit(1)
    }
    withExtendedLifetime(instanceLock) {
        let application = NSApplication.shared
        let applicationDelegate = AppDelegate()
        applicationDelegate.reopenRequests = reopenRequests
        application.delegate = applicationDelegate
        application.run()
    }
}

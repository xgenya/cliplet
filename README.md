...
- **外观与动画**：在设置中调节历史窗口的透明度与清晰度，并从聚焦、弹性、缩放等预设中选择打开动画，均可实时预览。
...
设置中的滑块可在「透明」与「清晰」之间调节历史窗口的玻璃。默认范围保证文字可读；开启「自由调节」后可使用更透明的材质。开启系统「降低透明度」时始终使用清晰的材质，开启「减弱动态效果」时打开动画改为淡入。

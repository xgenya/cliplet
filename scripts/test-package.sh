...
cp "$APP_DIR/Contents/MacOS/ClipboardNative" "$TEST_ROOT/bin/ClipboardNative"
cp -R "$APP_DIR/Contents/Resources/ClipboardNative_ClipboardNative.bundle" "$TEST_ROOT/bin/"
SOURCE_BUNDLE="$TEST_ROOT/bin/ClipboardNative_ClipboardNative.bundle"
...
[[ -f "$TARGET_APP/Contents/Resources/ClipboardNative_ClipboardNative.bundle/obsolete.txt" ]]
rm "$SOURCE_BUNDLE/obsolete.txt"
"$ROOT_DIR/scripts/package-app.sh" "$TEST_ROOT/bin" "$TARGET_APP" release
[[ ! -e "$TARGET_APP/Contents/Resources/ClipboardNative_ClipboardNative.bundle/obsolete.txt" ]]
# A failed package must not replace the previous valid application.
BEFORE="$(shasum -a 256 "$TARGET_APP/Contents/MacOS/ClipboardNative")"
